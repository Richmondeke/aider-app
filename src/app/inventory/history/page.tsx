        ...
        <div className="space-y-8 max-w-[1400px] mx-auto">
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                    <div className="bg-zinc-900 border border-zinc-800 p-3 rounded-2xl text-zinc-400">
                        <History size={24} />
                    </div>
                    <div>
                        <h1 className="text-3xl font-bold text-white tracking-tight">Stock History</h1>
            ...
            <div className="space-y-4">
                {logs.map((log) => (
                    <div
                        key={log.id}
                        className="group bg-zinc-900/40 border border-zinc-800 hover:border-zinc-700 p-6 rounded-[32px] flex flex-col md:flex-row md:items-center justify-between gap-6 transition-all"
                    >
                        <div className="flex items-center gap-5">
                            <div className={cn(
                                "w-14 h-14 rounded-[22px] flex items-center justify-center shrink-0",
                                log.type === "in" && "bg-emerald-500/10 text-emerald-500",
                                log.type === "out" && "bg-blue-500/10 text-blue-500",
                            ...
                            <div className="flex flex-col">
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-zinc-400 text-xs font-bold">{log.action}</span>
                                </div>
                                <h3 className="text-lg font-bold text-white mt-0.5">{log.product}</h3>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-12 text-right">
                            <div className="flex flex-col gap-0.5">
                                ...
                                <span className={cn(
                                    "text-xl font-black",
                                    log.type === "in" ? "text-emerald-500" : log.type === "out" ? "text-blue-500" : "text-amber-500"
                                )}>
                                    {log.quantity}
                                </span>
                            </div>
            ...
            <div className="py-10 text-center">
                <button className="text-blue-500 hover:text-blue-400 font-bold text-sm flex items-center gap-2 mx-auto px-6 py-3 rounded-2xl bg-blue-600/5 hover:bg-blue-600/10 transition-all">
                    Load more logs
                    <History size={16} />
                </button>
            </div>
