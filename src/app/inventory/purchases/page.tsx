...
export default function PurchasesPage() {
    return (
        <div className="space-y-8 max-w-[1600px] mx-auto">
            ...
            {/* Status Filter Tabs */}
            <div className="flex items-center gap-1 bg-zinc-900/50 border border-zinc-800 p-1.5 rounded-2xl w-fit">
                {["All Orders", "Pending", "Transit", "Received", "Cancelled"].map((tab) => (
                    <button
                        key={tab}
                        className={cn(
                            "px-5 py-2.5 rounded-xl text-xs font-bold transition-all",
                            tab === "All Orders" ? "bg-zinc-800 text-white" : "text-zinc-500 hover:text-zinc-300"
                        )}
                    >
                        {tab}
                    </button>
            ...
            <div className="grid grid-cols-1 gap-4">
                {orders.map((order) => (
                    <div
                        key={order.id}
                        className="group bg-zinc-900/40 border border-zinc-800 hover:border-zinc-700 rounded-[32px] p-8 flex flex-col lg:flex-row items-center justify-between gap-8 transition-all"
                    >
                        <div className="flex items-center gap-6 flex-1">
                            <div className={cn(
                                "w-16 h-16 rounded-3xl flex items-center justify-center shrink-0 border border-zinc-800",
                                order.status === "Received" && "bg-emerald-500/10 text-emerald-500",
                                order.status === "Pending" && "bg-amber-500/10 text-amber-500",
                                order.status === "Transit" && "bg-blue-500/10 text-blue-500",
                            ...
                            <div className="flex flex-col gap-1">
                                ...
                                <h3 className="text-xl font-black text-white">{order.vendor}</h3>
                        ...
                        <div className="flex items-center gap-12">
                            ...
                            <div className="flex flex-col gap-2 min-w-[140px]">
                                <span className={cn(
                                    ...
                                    order.status === "Transit" && "bg-blue-500/10 text-blue-500 border border-blue-500/20",
                                    order.status === "Cancelled" && "bg-red-500/10 text-red-500 border border-red-500/20",
                                )}>
                                    {order.status}
                                </span>
                                <button className="text-zinc-500 hover:text-white text-[10px] font-bold uppercase tracking-widest transition-all">
                                    View full PO
                                </button>
                            </div>
                            <button className="p-3 bg-zinc-950 border border-zinc-800 rounded-2xl text-zinc-500 hover:text-white hover:bg-zinc-800 transition-all">
