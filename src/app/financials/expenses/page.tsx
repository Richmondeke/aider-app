    ...
    return (
        <div className="space-y-8 max-w-[1600px] mx-auto">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-white tracking-tight">Financial Outflow</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    ...
                    <div key={stat.label} className="bg-zinc-900/40 border border-zinc-800 p-8 rounded-[40px] relative overflow-hidden">
                        <div className="relative z-10 flex flex-col h-full justify-between gap-6">
                            ...
                            <div>
                                ...
                                <p className="text-2xl font-black text-white mt-1 break-words">{stat.value}</p>
            ...
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-[40px] overflow-hidden">
                <div className="p-8 border-b border-zinc-800 flex flex-col md:flex-row md:items-center justify-between gap-6">
                    ...
                    <button className="bg-zinc-900 border border-zinc-800 text-zinc-300 px-8 py-3.5 rounded-2xl flex items-center justify-center gap-2 hover:bg-zinc-800 transition-all font-bold">
                        <Filter size={18} />
                        Filters
                    </button>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {expenses.map((exp) => (
                                <tr key={exp.id} className="group hover:bg-zinc-800/20 transition-all">
                                    <td className="px-10 py-8">
                                        <div className="flex items-center gap-5">
                                            <div className={cn(
                                                "w-14 h-14 rounded-2xl flex items-center justify-center text-xl font-black border",
                                                exp.type === "fixed" ? "bg-purple-600/10 border-purple-500/20 text-purple-500" : "bg-blue-600/10 border-blue-500/20 text-blue-500"
                                            )}>
                                                {exp.merchant[0]}
                                            </div>
                                            <div>
                                                <span className="text-zinc-100 font-bold block">{exp.merchant}</span>
                                                <span className="text-zinc-600 text-[11px] font-black uppercase tracking-widest">{exp.id}</span>
                                            </div>
                                        </div>
                                    </td>
                                    <td className="px-10 py-8">
                                        <span className="inline-flex items-center px-4 py-1.5 rounded-xl bg-zinc-800 text-zinc-400 text-[10px] font-black uppercase tracking-widest">
                                            {exp.category}
                                        </span>
                                    </td>
                                    <td className="px-10 py-8 text-zinc-400 font-bold text-sm">
                                        {exp.date}
                                    </td>
                                    <td className="px-10 py-8 font-black text-zinc-100">{exp.amount}</td>
                                    <td className="px-10 py-8">
                                        <span className={cn(
