    ...
    return (
        <div className="space-y-8">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-3xl font-black text-white tracking-tight">Accounting & Ledger</h1>
                    <p className="text-zinc-500 font-medium">Double-entry bookkeeping and financial reconciliation.</p>
                </div>
                <div className="flex items-center gap-3">
                    <button className="bg-blue-600 text-white px-6 py-3 rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-600/20 flex items-center gap-2">
                        <PlusIcon size={18} />
                        New Entry
                    </button>
                </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                {[
                    { label: "Equity", value: "₦12.8M", trend: "+4.2%", icon: BookOpen, color: "text-blue-500", bg: "bg-blue-600/10" },
                    ...
                    <div key={idx} className="bg-zinc-900/40 border border-zinc-800 p-6 rounded-[32px] space-y-4">
            ...
            <div className="bg-zinc-900/40 border border-zinc-800 rounded-[40px] overflow-hidden">
                <div className="p-8 border-b border-zinc-800 flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                        ...
                        <h3 className="text-lg font-black text-white">General Ledger</h3>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="bg-zinc-950 border border-zinc-800 rounded-xl py-2.5 pl-10 pr-4 text-xs font-bold text-zinc-300 outline-none w-64"
                            />
                        </div>
                        <button className="p-2.5 hover:bg-zinc-800 rounded-xl border border-zinc-800 text-zinc-400">
                            <Download size={18} />
                        </button>
                    </div>
                </div>
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="divide-y divide-zinc-800">
                            {ledgeEntries.map((entry) => (
                                <tr key={entry.id} className="hover:bg-zinc-800/30 transition-all group">
                                    <td className="py-6 px-8">
                                        <div className="flex flex-col">
                                            <span className="text-white font-bold text-sm tracking-tight">{entry.date}</span>
                                            <span className="text-zinc-500 text-[10px] font-black tracking-widest uppercase">{entry.id}</span>
                                        </div>
                                    </td>
                                    <td className="py-6 px-8">
                                        <div className="max-w-[300px]">
                                            <span className="text-zinc-300 font-medium text-sm">{entry.description}</span>
                                        </div>
                                    </td>
                                    <td className="py-6 px-8">
                ...
                <div className="p-10 border-t border-zinc-800 bg-zinc-950/20 text-center">
