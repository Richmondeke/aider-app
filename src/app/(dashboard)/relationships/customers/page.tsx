    ...
    return (
        <div className="space-y-8 max-w-[1600px] mx-auto">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-zinc-900 tracking-tight">Customer Network</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {[
                    ...
                    <div key={stat.label} className="bg-zinc-50/40 border border-zinc-200 p-8 rounded-[40px] flex items-center justify-between">
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left">
                        ...
                        <tbody className="divide-y divide-zinc-800/50">
                            {customers.map((cus) => (
                                <tr key={cus.id} className="group hover:bg-zinc-100/20 transition-all">
                                    <td className="px-10 py-8">
                                        <div className="flex items-center gap-5">
                                            <div className="w-14 h-14 rounded-2xl bg-zinc-100 border-2 border-zinc-700/50 flex items-center justify-center text-xl font-black text-zinc-500">
                                                {cus.name[0]}
                                            </div>
                                            <div>
                                                <span className="text-zinc-900 font-bold block">{cus.name}</span>
                                    ...
                                    <td className="px-10 py-8">
                                        <span className={cn(
                                            ...
                                            cus.tier === "VIP" && "bg-blue-500/10 text-red-600 border border-blue-500/20",
                                            cus.tier === "Standard" && "bg-zinc-500/10 text-zinc-500 border border-zinc-500/20",
                                        )}>
                                            {cus.tier}
                                        </span>
                                    </td>
                                    <td className="px-10 py-8 font-black text-zinc-700">{cus.orders}</td>
                                    <td className="px-10 py-8">
                                        <span className="text-zinc-900 font-black">{cus.spent}</span>
                                    </td>
                                    <td className="px-10 py-8">
                                        <div className="flex items-center gap-2">
                                            <div className="w-2 h-2 rounded-full bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]" />
                                            <span className="text-xs font-bold text-zinc-500">{cus.status}</span>
                                        </div>
                                    </td>
