...
const notifications = [
    ...
    { id: 3, type: "system", title: "System Update", message: "Aider Pro v1.1 is now live with new reports.", time: "5h ago", read: true, icon: Info, color: "text-blue-500", bg: "bg-blue-500/10" },
    ...
    return (
        <>
            ...
            <motion.div
                ...
                className="absolute right-0 mt-2 w-[400px] bg-zinc-900 border border-zinc-800 rounded-[32px] shadow-2xl z-[120] overflow-hidden"
            >
                <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-950/50">
                    ...
                    <button className="text-[10px] font-black text-zinc-500 hover:text-white uppercase tracking-widest flex items-center gap-1.5 transition-colors">
                        <CheckCheck size={14} />
                        Mark all as read
                    </button>
                </div>
                ...
                <div className="max-h-[480px] overflow-y-auto custom-scrollbar">
                    {notifications.map((notif) => (
                        <div
                            key={notif.id}
                            className={cn(
                                "p-6 flex gap-4 hover:bg-zinc-800/40 transition-all cursor-pointer border-b border-zinc-800/50 last:border-none",
                        ...
                        >
                            ...
                            <div className="flex-1 space-y-1">
                                <div className="flex items-center justify-between">
                                    <span className="text-sm font-bold text-white tracking-tight">{notif.title}</span>
                                    <span className="text-[10px] font-black text-zinc-600 uppercase italic">{notif.time}</span>
                                </div>
                                <p className="text-xs text-zinc-400 font-medium leading-relaxed">{notif.message}</p>
                            </div>
                            {!notif.read && (
                ...
                <div className="p-4 bg-zinc-950/50 border-t border-zinc-800 text-center">
                    <button className="text-[10px] font-black text-zinc-500 hover:text-white uppercase tracking-widest transition-all">
